...
module.exports = async ({query},res)=>{
    ...
    console.log(query)
    let {ticket, service} = query
    karavaki()
        .then(async db=>{
            let authenticationRequest = await db.collection('tickets').findOne({ticket, service})
            let user = await db.collection('users').findOne({email: authenticationRequest.email})
